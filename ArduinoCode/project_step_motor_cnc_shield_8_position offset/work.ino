...
double offset_arm=-180, offset_boom=-90, offset_base=0;
double posx = 12.0, posy = 12.0, posz=0.0;
double boomangle_th1,armangle_th2,th21;

void startliner();//초기 시작 오프셋 각도 설정
void modeselect();//매뉴얼 컨트롤러 조이스틱 인식시 수동 동작
void modeselect2();//매뉴얼 컨트롤러 버튼 인식시 수동 동작
...
//  modeselect2();
  ...
  anglemovementinset(45,45,45);
  currentpositionshower();
  delay(500);

  anglemovementinset(90,90,90);
  currentpositionshower();
  delay(500);
...
void manualcontroller3()
{
  int adder = 10;
  int x = digitalRead(xval);
  
  if(x == 0) positions[robot_position_arm] -= adder;
  else{}

//  if(y == 0) positions[robot_position_arm] += adder;
//  else{}
