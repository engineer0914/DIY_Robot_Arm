...
패치노트
  ...
  7. 모드 설정 및 조이스틱 무브먼트 개선 = 시작과 동시에 조이스틱 입력시 모드전환
  6. 각도 입력 전 초기 위치 설정
  5. 감속기어 부착 버전 각도 입력 및 동시 도착
  4. 각 모터별 각도 입력 및 동시 도착
  3. 조이스틱 무브먼트 속도 개선, 설정 가능
  2. 동시동작 동시 도착
  1. 조이스틱 무브먼트
...
int minval = 300, maxval = 700, swstate = 0;
long positions[3];
double armangle_current, boomangle_current, baseangle_current;
double posx = 12.0, posy = 12.0, posz=0.0;
double boomangle_th1,armangle_th2,th21;

void startliner();//초기 시작 오프셋 각도 설정
void modeselect();//매뉴얼 컨트롤러 인식시 수동 동작
void manualcontroller2();//매뉴얼 컨트롤러 모드
...
{
  modeselect();
//  anglemovementinset(0,0,0);
//  delay(500);
//  
//  anglemovementinset(0,45,45);
  delay(500);
}
...
2.z,x축 2차원 좌표 산정
  zx평면 회전 각도 th_z = atan(P_z/(P_x - wd))
  zx평면 인장 x좌표 P_xr = sqrt(pow((P_x - wd),2) + pow(P_z,2))

3.xy축 2차원 좌표 선정
연장 좌표
D = pow((P_xr - wd),2) + pow(P_y,2))



//역기구학 각도 계산
void Inverse_Kinematics_Calculator(double P_x, double P_y, double P_z)
{
  //조금 다른 방식으로 접근
  
  currentpositionshower();//현재 위치 산출
  
  //역기구학 ppt에 있는 그림을 이용해서 보면 편함
  double L1 = 12.0;//boom의 길이
  double L3 = 12.0;//arm의 길이
  double wd = 3.0;//arm의 끝과 엔드이펙터 사이의 거리
  double D = sqrt(pow(P_x, 2) + pow(P_y, 2));//엔드이펙터 관절의 3차원 공간 위치
//  double posD = P_x + wd;//엔드이펙터의 3차원 공간 위치
  ...
  //th21값 계산(cos 2법칙 사용)
  th21 = acos((pow(L1, 2) + pow(L3, 2) - pow(D, 2)) / (2 * L1 * L3));

//  //th1을 계산하기 위해 삼각형 p0,p1,p4에서 각p1-p0-p4의 각도값 계산
//  double alpha = acos((pow(L1, 2) + pow(D, 2) - pow(L3, 2)) / (2 * L1 * D));
//  //th1을 계산하기 위해 삼각형 p0,p4_y,p4_x에서 각p4_y-p0-p4_x의 각도값 계산
//  double psi = acos((pow(D, 2) + pow(P_x - wd, 2) - pow(P_y, 2)) / (2 * D * (P_x - wd)));

  double boomangle_th1 = 아탄2함수로 만들기
  //붐

  //라디안->도 변환
  boomangle_th1 = boomangle_th1 * 180 / PI;
  ...
  anglemovementinset(0, boomangle_th1-boomangle_current, armangle_th2-armangle_current);
  steppers.moveTo(positions); //포지션 입력
  steppers.runSpeedToPosition(); //각각 동작


  Serial.println();
  Serial.print("final posD:");
  Serial.println(posD);
  Serial.print("D:");
  Serial.println(D);
  ...
  Serial.println(boomangle_th1);
  Serial.print("arm angle(th2):");
  Serial.println(armangle_th2);
  Serial.println();
}
*/
//void Inverse_Kinematics_Calculator(double P_x, double P_y)//역기구학 각도 계산
//{
//  currentpositionshower();//현재 위치 산출
//  
//  //역기구학 ppt에 있는 그림을 이용해서 보면 편함
...
//  double L3 = 12.0;//arm의 길이
//  double wd = 3.0;//arm의 끝과 엔드이펙터 사이의 거리
//  double D = sqrt(pow((P_x) - wd, 2) + pow(P_y, 2));//엔드이펙터 관절의 3차원 공간 위치
...
void anglemovementinset(double angle_base, double angle_boom, double angle_arm)
{
  ...
  instantba = (double)(angle_base*smallgearstep*microstep*normgear/360);
  instantar = (double)(angle_arm*smallgearstep*microstep*planetarygear*normgear/360);
  instantbo = (double)(angle_boom*smallgearstep*microstep*planetarygear*normgear/360);
...
void currentpositionshower()
{
  ...
  Serial.print("angle current base:");
...
void startliner()
{
  boom.setCurrentPosition(-2848);//오프셋 90도
  arm.setCurrentPosition(-5696);//오프셋 180도

//  boom.setCurrentPosition(-1424);//오프셋 45도
//  arm.setCurrentPosition(-1424);//오프셋 45도

  Serial.print("\n\nRobot Arm initiate angle set\n");
  
  currentpositionshower();

  Serial.print("\n\nRobot Arm initiate position set\n");
  Serial.print("X:");
  Serial.println(posx);
  Serial.print("Y:");
  Serial.println(posy);
