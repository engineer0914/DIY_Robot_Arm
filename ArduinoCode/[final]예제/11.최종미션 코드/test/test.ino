 ...
 *
 * Foward, Inverse Kinematics Final Complete Version
 */

  // 최종 미션 : 종이컵 위에 지우개 두고, 천천히 진입해서 집고 옮기는 코드
...
void perfomance_1()//역기구학 수직이동
{
  Inverse_Kinematics_Action(0,15,0);
  delay(3000);
  Inverse_Kinematics_Action(0,15,12);
  delay(3000);
  Inverse_Kinematics_Action(0,15,24);//원위치
  delay(3000);
}

void perfomance_2()//servo 확인
{
  // 0도로 이동
  myServo.write(0);
...
void perfomance_3()
{
  ...
  //  회전
  Inverse_Kinematics_Action(-15,15,24);
  delay(tttime);

  //  종이컵 앞으로 이동
  for(float i = 24;i>8;i=i-0.5)
  {
    Inverse_Kinematics_Action(-15,15,i);
  }
  delay(tttime);
  ...
  // 속도 낮춰서
  spdmotor = 2000;
  boom.setMaxSpeed(spdmotor);
  arm.setMaxSpeed(spdmotor);

  // 수평 전진 이동
  for(float i = 15;i<19;i=i+0.5)
  {
    Inverse_Kinematics_Action(-15,i,8);
  }  
  //  Inverse_Kinematics_Action(-15,19,8);
  delay(tttime);
  ...
  // 수직 상승 이동
  Inverse_Kinematics_Action(-15,19,12);
  delay(tttime);
  ...
  // 회전
  Inverse_Kinematics_Action(-78,19,12);
  delay(tttime);

  // 수직 하강
  Inverse_Kinematics_Action(-78,19,1);
  delay(tttime);
...
void Inverse_Kinematics_Calculator(double P_x, double P_y)
{
  ...
  P_y -= y_offset;//오프셋 제거
  P_x -= endeffectoroffset_wd;//오프셋 제거

  //2. 피타고라스 정리를 이용하여 D 계산 1
  D = sqrt(pow(P_x, 2) + pow(P_y, 2));//엔드이펙터 관절의 3차원 공간 위치
  ...
  alpha = acos((pow(boom_L1, 2) + pow(D, 2) - pow(arm_L3, 2)) / (2 * boom_L1 * D));

  //5. 제2 코사인 법칙을 통해 지면과 붐 사이 각(beta = psi) 구하기 1
  psi = acos((pow(D, 2) + pow(P_x, 2) - pow(P_y, 2)) / (2 * D * P_x));

  //6. 계산된 3개의 라디안 도 변환 (deg = rad*180/PI) 3 
  alpha = degrees(alpha);
  psi = degrees(psi);
  th21 = degrees(th21);//붐 암 사이 각

  //7. 붐 중앙축 y좌표 기준 엔드이펙터의 y좌표가 이상(alpha + psi)인지 이하(alpha - psi)별 수식 작성 = 붐 각도(boomangle_th1) 계산 2
  if (P_y >= 0) boomangle_th1 = alpha + psi;//만약 엔드이펙터의 높이가 boom,arm 모터의 중심보다 위에 있을경우
  else boomangle_th1 = alpha - psi;//만약 엔드이펙터의 높이가 boom,arm 모터의 중심보다 아래에 있을경우
