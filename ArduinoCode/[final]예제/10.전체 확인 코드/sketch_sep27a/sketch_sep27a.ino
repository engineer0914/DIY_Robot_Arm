...
void Back_Lash_caculator(double input_x_min,double input_y_min,double input_x_max,double input_y_max);
void perfomance_check();//기능 확인
...
void perfomance_check()
{
  ...
  for(;;)
  {
    for(;;)
    {
      ...
      if(digitalRead(gbuf)!=1) modenum = 7;
      if(digitalRead(gbub)!=1) modenum = 8;
      if(modenum != 0) break;
    }
  ...
  for(int i =0;i<20;i++)
  {
    rotationmovementinsert(i*50,i*50,i*50);
    delay(10);
  }
  for(int i = 20;i>0;i--)
  {
...
void Back_Lash_caculator(double input_x_min,double input_y_min,double input_x_max,double input_y_max)
{
  double instbacklash_min_b,instbacklash_min_a,instbacklash_max_b,instbacklash_max_a;

  Serial.print("back lash caculator\n");
  Serial.println();

  Serial.print("back lash minimum input x coordinate:");
  Serial.println(input_x_min);
  Serial.print("back lash minimum input y coordinate:");
  Serial.println(input_y_min);

  //  Inverse_Kinematics_Calculator(15.5,23.5);
  Inverse_Kinematics_Calculator(input_x_min,input_y_min);
  instbacklash_min_b = abs(out_boomangle);
  instbacklash_min_a = abs(out_armangle);
  Serial.print("back lash minimum angle boom:");
  Serial.println(instbacklash_min_b);
  Serial.print("back lash minimum angle arm:");
  Serial.println(instbacklash_min_a);

  Serial.println();

  Serial.print("back lash maximum input x coordinate:");
  Serial.println(input_x_max);
  Serial.print("back lash maximum input y coordinate:");
  Serial.println(input_y_max);
  
  //  Inverse_Kinematics_Calculator(14.5,24.5);
  Inverse_Kinematics_Calculator(input_x_max,input_y_max);
  instbacklash_max_b = abs(out_boomangle);
  instbacklash_max_a = abs(out_armangle);
  Serial.print("back lash maximum angle boom:");
  Serial.println(instbacklash_max_b);
  Serial.print("back lash maximum angle arm:");
  Serial.println(instbacklash_max_a);

  Serial.println();
  Serial.print("------------------------------------\n");
  Serial.print("TOTAL Back Lash Angle BOOM: ");
  Serial.println(instbacklash_min_b + instbacklash_max_b);
  Serial.print("TOTAL Back Lash Angle ARM: ");
  Serial.println(instbacklash_min_a + instbacklash_max_a);
  Serial.print("------------------------------------");
