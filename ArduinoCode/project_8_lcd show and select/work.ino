...
패치노트
  ...
  18. 버튼 컨트롤러 활성화, 특정 버튼 만 눌렀을때 컨트롤러 활성화
  17. 포트 입력 구분 및 컨트롤러 활성화 스위치 지정 및 생성
  16. 전처리 구문 분리
  15. ERRATA 추가 메모
  14. 시리얼 입력을 통해서 좌표 이동 모드 추가
  13. 설명 모드 추가, on off 정의 및 추가후 시인성 증가
  12. 관절별 포지션 주소 지정 오류 수정
  11. 모드 변경 버튼 조이스틱 통합
  10. 역기구학 구현
  09. 정기구학 구현 + 중도 오류 수정
...
ERRATA
  ...
  1번 모터가 돌아야 하는데 2번 모터가 돈다? = position 배열 숫자 수정 할 것
  1번 모터의 위치 값이 수정되지 않는다? = 객체 선언 오류
  1번 모터가 부르르 떨면서 덜 돌거나, 돌다가 좌우로 방향이 자꾸 바뀐다 = 모터 선 끊어짐 + 드라이버 문제
  드라이버도 쉽게 타지는 않음
  풀스텝 마이크로스텝 모드 자주 변경 금지 - 아마 플로팅 때문에 모드 고정 불가 추정
  아두이노 우노 메가 호환 가능
*/
...
#define serialcontroller    ON      //serial cotroller 0 off 1 on
#define handcontroller      ON      //0 joystick 1 button
...
void lcdcountdownshower();
void perfomance_1();
void perfomance_2();
void showselector();//lcd 연결시 모드 선택 화면 보이기
...
void showselector()
{
  ...
  lcd.clear();
  lcd.setCursor(9,0);
  lcd.print("1.hand");//1.핸드 컨트롤러
  lcd.setCursor(0,0);
  lcd.print("2.uart");//2.시리얼 컨트롤러 - 진입시 키 입력 확인
  lcd.setCursor(9,1);
  lcd.print("3.pfm1");//3.시연1번모드 - 픽앤플레이스
  lcd.setCursor(0,1);
  lcd.print("4.pfm2");//4.시연2번모드 - 역기구학
  for(;;)
  {
    if(digitalRead(xbuf)!=1)
    {
      modenum = 1;
      break;
    }
    if(digitalRead(xbub)!=1)
    {
      modenum = 2;
      break;
    }
    if(digitalRead(ybuf)!=1)
    {
      modenum = 3;
      break;
    }
    if(digitalRead(ybub)!=1)
    {
      modenum = 4;
      break;
    }
...
*/
  ...
  Inverse_Kinematics_Action(0,15,24);
  delay(100*j);
  Inverse_Kinematics_Action(0,15,20);
  delay(100*j);
  Inverse_Kinematics_Action(0,10,15);
  delay(100*j);
  Inverse_Kinematics_Action(0,15,10);
  delay(100*j);
  Inverse_Kinematics_Action(0,20,15);
  delay(100*j);
  Inverse_Kinematics_Action(0,15,20);
  delay(100*j);
...
*/
  ...
  Inverse_Kinematics_Action(0,15,20);
  delay(100*j);
  Inverse_Kinematics_Action(0,20,20);
  delay(100*j);
  Inverse_Kinematics_Action(0,20,10);
  delay(100*j);
  Inverse_Kinematics_Action(0,10,10);
  delay(100*j);
  Inverse_Kinematics_Action(0,10,20);
  delay(100*j);
  Inverse_Kinematics_Action(0,15,20);
  delay(100*j);
  Inverse_Kinematics_Action(0,15,24);
...
void modeselect()
{
  ...
  else if(modenum == 5)//joystick control
  {
    ...
    Serial.print("\n initiate robotArm manual mode \n press reset to exit manual mode and reset robotArm");
    Serial.println();
    Serial.print("joystick controller enable:");
